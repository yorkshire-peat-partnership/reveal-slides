...
##colnames(site_peat_units) <- c("Site","Estate")


tm <- tm_shape(site_peat_units) + tm_fill(col='brown') + tm_layout(frame = FALSE, bg.color = "transparent", outer.margins = 0.2)
tm1 <- tm_basemap(leaflet::providers$OpenStreetMap) + tm_shape(unit_status) + tm_polygons(id="site_name", col = "full_desc", title = "Site progress")
